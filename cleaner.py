        ...
        with urllib.request.urlopen(GITHUB_RAW_URL, timeout=3) as response:
            ...
            if match:
                ...
                if remote_version > VERSION:
                    ask = subprocess.run([
                        "zenity", "--question",
                        "--title", "The update is available",
                        "--text", f"A new version of {remote_version} is available (you have {VERSION}). Update?"
                    ])

                    if ask.returncode == 0:
                        with open(__file__, "w") as f:
                            f.write(content)
                        subprocess.run(["zenity", "--info", "--text", "The script has been updated! Please rerun it."])
                        sys.exit(0)
            else:
        ...
        if os.path.exists(log_path):
            with open(log_path, "r", errors="ignore") as f:
                for line in reversed(f.readlines()):
                    if appid in line:
                        match = re.search(r"AppID\s\d+,\s(.*)", line) or re.search(r"AppId=(\d+)\s(.*)", line)
                        if match: return match.groups()[-1].strip()
    return "Unknown"
...
def collect_data(target_type, library_paths, api_names):
    ...
    for appid in os.listdir(target_dir):
        ...
        if not os.path.isdir(full_path) or not appid.isdigit(): continue
        if "Proton" in appid: continue
        ...
        if not name: name = get_name_from_logs(appid)
        if "Proton" in (name or ""): continue

        size = get_folder_size(full_path)
        info_list.append(["FALSE", str(size), appid, name or "Unknown", info, full_path])
...
def main(mode="shadercache"):
    ...
    cmd = [
        "zenity", "--list", "--checklist", "--title", f"Cleanup {mode} (v{VERSION})",
        "--width=1100", "--height=720", "--print-column=6", "--separator=|",
        "--column=Choice", "--column=Size (MB)", "--column=ID", "--column=Name", "--column=Info", "--column=Path",
        "--extra-button", next_mode
    ]
    for row in data: cmd.extend(row)

    proc = subprocess.run(cmd, capture_output=True, text=True)
    res = proc.stdout.strip()

    if proc.returncode == 1:
        if res == next_mode: main(next_mode)
        return

    paths_to_delete = res.split("|")
    if not paths_to_delete or paths_to_delete == ['']: return

    if subprocess.run(["zenity", "--question", "--text", f"Delete {len(paths_to_delete)} folders?"], capture_output=True).returncode == 0:
